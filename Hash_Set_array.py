...
class Hash_Set:
    ...
    def insert(self, value):
        ...
        if self._table[x]._linear_search(value)==-1:
            if len(self._table[x])<self._LOAD_FACTOR:
                self._table[x].append(deepcopy(value))
                self._count+=1
            else:
                self._rehash()
                x=hash(value)%self._capacity
    ...
    def find(self, key):
        ...
        """
        value=None
        x=hash(key)%len(self._table)
        if key in self._table[x]:
            value=self._table[x].find(key)
    ...
    def __eq__(self, target):
        ...
        """
        equals=True
        if len(self._table)==len(target._table):
            for i in range(len(self._table)):
                if len(self._table[i])==len(target._table[i]):
                    z=0
                    while z<len(self._table[i]):
                        if self._table[i][z]!=target._table[i][z]:
                            equals=False
                        z+=1
                else:
                    equals=False
