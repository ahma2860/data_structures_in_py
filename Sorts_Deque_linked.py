...
class Sorts:
    ...
    def gnome_sort(a):
        ...
        -------------------------------------------------------
        """
        pointer_front=a._front._next
        Sorts._gnome_sort_aux(a, pointer_front)
        
        return 

    @staticmethod
    def _gnome_sort_aux(a, pointer_front):
        
        if pointer_front:
            print(f"pointer at {pointer_front._value}")
        else:
            print(f"pointer front is None")
        input()     
        

        if pointer_front:
            pointer_left = pointer_front._prev
            if pointer_left is None:
                Sorts._gnome_sort_aux(a, pointer_front._next)
            else:
                if pointer_front._value<pointer_left._value:
                    Sorts.swaps+=1
                    a._swap(pointer_left, pointer_front)
                    Sorts._gnome_sort_aux(a, pointer_front._prev)
                elif pointer_front._value>=pointer_left._value:
                    Sorts._gnome_sort_aux(a, pointer_front._next)
