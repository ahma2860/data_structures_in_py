...
class Sorts:
    ...
    def gnome_sort(a):
        ...
        Sorts._gnome_sort_aux(a, 1)
            
        return 

    @staticmethod
    def _gnome_sort_aux(a, pointer_front):
        """print(f"pointer_front: {pointer_front}")
        print(f"{a}")
        input()     soooooooooooooooooooooooooooooooo helpfulllllllllll"""
        
        pointer_left = pointer_front - 1
        if pointer_front < len(a):
            if pointer_front==0:
                pointer_front+=1
                Sorts._gnome_sort_aux(a, pointer_front)
            else:
                if a[pointer_front]<a[pointer_left]:
                    Sorts._swap(a, pointer_left, pointer_front)
                    Sorts._gnome_sort_aux(a, pointer_front-1)
                elif a[pointer_front]>=a[pointer_left]:
                    Sorts._gnome_sort_aux(a, pointer_front+1)
